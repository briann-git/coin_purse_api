...
def test_create_budget_item_returns_201(client: TestClient, user_id: str, budget: dict, category: dict):
    res = client.post(
        f"/users/{user_id}/budgets/{budget['id']}/items",
        json={"category_id": category["id"], "limit_amount": "500.00"},
    )
...
def test_create_budget_item_unknown_category_returns_404(client: TestClient, user_id: str, budget: dict):
    res = client.post(
        f"/users/{user_id}/budgets/{budget['id']}/items",
...
def test_list_budget_items(client: TestClient, user_id: str, budget: dict, budget_item: dict):
...
def test_get_budget_item(client: TestClient, user_id: str, budget: dict, budget_item: dict):
    res = client.get(f"/users/{user_id}/budgets/{budget['id']}/items/{budget_item['id']}")
    assert res.status_code == 200
    assert res.json()["id"] == budget_item["id"]


def test_get_budget_item_not_found_returns_404(client: TestClient, user_id: str, budget: dict):
...
def test_update_budget_item_limit(client: TestClient, user_id: str, budget: dict, budget_item: dict):
    res = client.patch(
...
def test_delete_budget_item_returns_204(client: TestClient, user_id: str, budget: dict, budget_item: dict):
    res = client.delete(f"/users/{user_id}/budgets/{budget['id']}/items/{budget_item['id']}")
    assert res.status_code == 204


def test_deleted_budget_item_not_in_list(client: TestClient, user_id: str, budget: dict, budget_item: dict):
    client.delete(f"/users/{user_id}/budgets/{budget['id']}/items/{budget_item['id']}")
    ids = [i["id"] for i in client.get(f"/users/{user_id}/budgets/{budget['id']}/items").json()]
...
def test_flat_list_filter_by_budget_id(client: TestClient, user_id: str, budget: dict, budget_item: dict):  # noqa: ARG001
    res = client.get(f"/users/{user_id}/budget-items?budget_id={budget['id']}")
    assert res.status_code == 200
    assert all(i["budget_id"] == budget["id"] for i in res.json())


def test_flat_list_filter_wrong_budget_id_returns_empty(client: TestClient, user_id: str, budget_item: dict):  # noqa: ARG001
...
def test_flat_update_budget_item(client: TestClient, user_id: str, budget_item: dict):
    res = client.patch(f"/users/{user_id}/budget-items/{budget_item['id']}", json={"limit_amount": "750.00"})
    assert res.status_code == 200
